...
| `CHANGELOG.md` | Keep-a-Changelog format; pre-1.0 semver per ADR-0003. Current `0.11.0` (source of truth: `package.json`; `release-please` drives bumps). |
...
| `index.html` (root) | The **UI Kit Dashboard** — the top-level standalone surface (~108KB). Its version label is stamped by `scripts/version-stamp.mjs` and CI-guarded (`stamp-check.yml`). Not to be confused with `review/index.html`. As of #21 it **consumes the `components/` registry** (`.tk-*` classes) instead of page-local component CSS; `.kit-card` stays bespoke (no registry equivalent). |
...
| `adr/` | Twelve accepted ADRs — the load-bearing decisions written down. |
| `components/` | **Component registry (stage d, ADR-0011/0012).** One dir per component (`<name>/<name>.{css,html,hbs}`) = one future Ghost partial bundle. Token-only `.tk-*` CSS with `data-tk-slot` / `data-variant` / `data-pillar` / `data-style` axes. The `tk-components.css` barrel `@import url()`s them all (the `url()` form is mandatory for font-guard). `.html` is the ADR-0011 reference specimen; `.hbs` is the Ghost rendering. Built in groups: 5 primitives + composites C1 structural / C2 data&forms / C3 cards / C4 signature flywheel / C5 commerce / C6 distribution / C7 membership — **34 components total**. |
| `Component Gallery.html` (root) | Single-page catalog (ADR-0012): live theme-following specimens + per-component tabbed **HTML · CSS · Ghost** copy. Reuses study-page chrome + `_shared/tk-theme.*` toggle. Static `jun 2026` label, intentionally outside `version-stamp` `FILES`. The `COMPONENTS` JS array (id/idx/name/family + html/css/hbs template-literal strings) drives it. |
...
The repo is governed by twelve ADRs in `adr/` — each is short and load-bearing:
...
- **ADR-0011 · Component-model conventions.** The binding model for the `components/` registry (stage d, the shadcn-style component library): token-only CSS (`var(--tk-*)` only — no raw hex, literal px confined to 1px hairlines / tokenized radii / breakpoints / SVG coordinate geometry), `data-tk-slot` sub-parts, attribute axes `data-variant` / `data-pillar` / `data-style`, **bare class = default variant**, no `!important`, no inline styles for themable props.
- **ADR-0012 · Gallery & copy-code contract.** `Component Gallery.html` is the single-page catalog with per-component tabbed **HTML · CSS · Ghost (`.hbs`)** copy. Each component ships `components/<name>/<name>.{css,html,hbs}`; the `components/tk-components.css` barrel pulls them via `@import url('./…')` — **the `url()` form is REQUIRED** (`scripts/font-guard.mjs` only follows `@import url()` when validating the `var(--tk-*)` chain; a bare `@import "…"` is invisible to CI). The `.hbs` is a sanctioned third rendering of the same DOM contract for the future Ghost theme; no fictional Ghost helpers (themes can't define custom helpers — would fail GScan).
...
- **commitlint (husky `commit-msg`) rejects uppercase/sentence-case subjects.** The subject after `type(scope):` must **start lowercase** — `feat(components): commerce composites …`, never `feat(components): C5 commerce …` (a leading `C5`/`Add`/capital fails `subject-case`). Header ≤100 chars and each body line ≤100 chars too. Put batch labels like "C5" in the body, not the subject start. Applies to both this repo and the downstream mockup repo.
- **`components/` registry feeds a downstream repo — `Tekrogen-Ghost-theme-mockup`.** This DS repo is the source of truth; the registry (`components/`) is vendored into the mockup repo (`cp -R components/. …`) where the `Claude-DT` hi-fi mockup migrates its bespoke markup onto the `.tk-*` classes. That repo has commitlint but **no committed CI** (PRs merge manually) and a manual version ritual (bump 5 markers — `index.html` `<meta name="version">` + `.ver` badge, `README.md` H1, `Brand Compliance Checklist.md` H1, `CHANGELOG.md` — plus the `Claude-DT` mockup's own version line — then tag + GitHub release). The per-group rhythm: merge the DS PR → re-vendor → migrate the mockup → merge that PR → cut the mockup release.
- **`CHANGELOG.md` is release-please-managed after the first release — don't hand-edit it.** Pre-1.0 the repo kept rich manual `[Unreleased]` entries, but release-please now regenerates the changelog from commit messages on every Release-PR update (its own format: `### Features`, commit links). So the ADR-0003 required documentation — **versioning intent, pixel-diff scope, migration, assets-to-regenerate** — goes in the **commit body + PR body** (which release-please surfaces), NOT a manual CHANGELOG edit (it would be overwritten). Manual `[Unreleased]` curation only applies pre-first-release or in the manual-tag mockup repo.
- **A visual change ⇒ `feat:` (minor), even when the work is a refactor.** ADR-0003 makes any visual change MINOR, but release-please only bumps on `feat:` / `fix:` / breaking — `refactor:` is no-release. So a registry-adoption "refactor" that carries even a ≤2px pixel-diff must be committed (and PR-titled) as **`feat:`**, or the visual change ships unversioned and violates ADR-0003. Name the branch `feat/<issue#>-…` to match (Rule #3), and flag the type override to the user. (Precedent: #21 — `refactor(dashboard): adopt the registry` → shipped as `feat(dashboard): …`.)
...
3. **Always follow the branch-naming convention, for ALL projects.** Every branch is `<type>/<issue#>-<slug>` (e.g. `feat/42-oauth-login`, `fix/57-api-timeout`) — a conventional-commit `<type>`, the GitHub issue it closes, and a short slug. No issue yet? Open one first; the branch must trace to a goal. The PR title is itself a conventional commit and the body says `Closes #<issue#>`. This is non-negotiable across every Tekrogen repo. The canonical rules live in the cross-project library at `/Volumes/SERV01-DTMAC/_Code_Library/GIT-Workflows` (`BRANCH-WORKFLOWS.md`, `incrementally-commit-based-on-file-creation.md`) — read them before branch/commit/release work.
4. **Release engine: `release-please` is the default for everything; `release-it` is only for NPM packages.** Per the `GIT-Workflows` library, pick **one engine per repo** — `release-please` (CI-driven, standing Release PR) is the default for all non-package repos (this design system, the Ghost theme mockup, static sites, theme bundles); `release-it` (manual `npx release-it`) is reserved mainly for published NPM packages. Never install both in one repo. Reference any PR/issue with the repo named (e.g. "DS PR #44", "mockup issue #3") since each repo has independent numbering.